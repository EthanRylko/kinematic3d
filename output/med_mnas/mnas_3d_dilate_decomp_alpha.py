...
class RPN(nn.Module):
    ...
    def __init__(self, phase, base, conf):
        ...
        self.base[12][0].layers[3].stride = (1, 1)

        out_channels = base[-3].out_channels
        ...
        # outputs
        self.cls = nn.Conv2d(out_channels, self.num_classes * self.num_anchors, 1)

        # bbox 2d
        self.bbox_x = nn.Conv2d(out_channels, self.num_anchors, 1)
        self.bbox_y = nn.Conv2d(out_channels, self.num_anchors, 1)
        self.bbox_w = nn.Conv2d(out_channels, self.num_anchors, 1)
        self.bbox_h = nn.Conv2d(out_channels, self.num_anchors, 1)

        # bbox 3d
        self.bbox_x3d = nn.Conv2d(out_channels, self.num_anchors, 1)
        self.bbox_y3d = nn.Conv2d(out_channels, self.num_anchors, 1)
        self.bbox_z3d = nn.Conv2d(out_channels, self.num_anchors, 1)
        self.bbox_w3d = nn.Conv2d(out_channels, self.num_anchors, 1)
        self.bbox_h3d = nn.Conv2d(out_channels, self.num_anchors, 1)
        self.bbox_l3d = nn.Conv2d(out_channels, self.num_anchors, 1)

        self.bbox_alpha = nn.Conv2d(out_channels, self.num_anchors, 1)
        self.bbox_axis = nn.Conv2d(out_channels, self.num_anchors, 1)
        self.bbox_head = nn.Conv2d(out_channels, self.num_anchors, 1)
...
def build(conf, phase):
    ...
    base = models.mnasnet1_0(pretrained=train)

    rpn_net = RPN(phase, base.layers, conf)
